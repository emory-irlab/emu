...
public class DBUtil {
	...
	protected static String _database = "emu";
	...
	public DBUtil() {
		...
		String database = "postgres";
		String user = "postgres";
		String pwd = "lagoon12345";
		init(postgresUrl, database, postgresClassName, user, pwd);
	}

	public DBUtil(String url, String database, String driverClassName,
			String user, String pwd) {
		init(url, database, driverClassName, user, pwd);
	}

	public void init(String url, String database, String driverClassName,
			String user, String pwd) {
		_database = database;
		_url = url;
		...
		try {
			...
			if (_stmt_insert_content == null || _stmt_insert_content.isClosed()) {
				...
				_stmt_insert_content = _conn
						.prepareStatement(_sql_insert_content);
			}
	...
	public PreparedStatement _stmt_insert = null;

	public void prepareQueryInsert(String sql) throws SQLException {
		_stmt_insert = _conn.prepareStatement(sql);
	}
	...
	public ResultSet executeQuerySQL(String sql) {
		...
		try {
			Statement stmt = _conn.createStatement();
			stmt = _conn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE,
	...
	boolean checkIfPageExists(String page_sha1) {
		validateConn();
		ResultSet rs = executeQuerySQL("select * from "+prefix+"_emu_pages where page_sha1='"
				+ page_sha1 + "'");
	...
	public void createPageTable(boolean dropTable) {
		if (dropTable) {
			this.execute("DROP TABLE emu_android_events;");
		}
		HashMap<String, String> tableFieldType = new HashMap<String, String>();
		// ---------------------
		tableFieldType.put("page_sha1", "text");
		tableFieldType.put("time", "timestamp");
		tableFieldType.put("log_time", "timestamp");
		tableFieldType.put("wid", "text");
		tableFieldType.put("tab", "text");
		tableFieldType.put("type", "text");
		tableFieldType.put("url", "text");
		tableFieldType.put("ref", "text");
		tableFieldType.put("ip", "text");
		tableFieldType.put("user_id", "integer");
		tableFieldType.put("content_id", "integer");
		tableFieldType.put("is_search", "boolean");
		tableFieldType.put("engine", "text");
		tableFieldType.put("vertical", "integer");
		tableFieldType.put("page_id", "bigint");

		// add more fields here...
		String sql = "CREATE TABLE emu_android_events ( ";
		sql += "page_id serial NOT NULL, ";
		for (String name : tableFieldType.keySet()) {
			sql += name + " " + tableFieldType.get(name) + ", ";
		}
		sql = sql.substring(0, sql.length() - 1) + ""
				+ "CONSTRAINT eventId PRIMARY KEY (eventId)" + ");";
		// ---------------------
		if (dropTable) {
			this.execute(sql);// create the events table
		}

	}

	String _sql_insert_page = "insert into "+prefix+"_emu_pages (" + "page_sha1, "
			+ "time, " + "wid, " + "tab, " + "type, " + "url, " + "ref, "
			+ "ip, " + "user_id, " + "is_search, " + "engine, " + "vertical, "
			+ "content_fn " + ") values (?,?,?,?,?,?,?,?,?,?,?,?,?)";

	public boolean insertPage(String page_sha1, Long time, String wid,
			String tab, String type, String url, String ref, String ip,
			Integer userID, Boolean isSearch, String engine, String vertical,
			String content_fn) throws UnknownHostException {
		...
		try {
			...
			_stmt_insert_page.setString(13, content_fn);
	...
	String _sql_insert_event = "insert into "+prefix+"_emu_events (" + "event_sha1, "
			+ "time, " + "ev, " + "url, " + "ref, " + "ip, " + "user_id, "
			+ "content_sha1, " + "cx, " + "cy, " + "x, " + "y, " + "scrlX, "
			+ "scrlY, " + "iw, " + "ih, " + "ow, " + "oh, " + "scrlW, "
			+ "scrlH, " + "dom_path, " + "targ_id, " + "is_doc_area, "
			+ "duration, " + "select_text, " + "button "
			+ ") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

	boolean checkIfEventExists(String event_sha1) {
		validateConn();
		ResultSet rs = executeQuerySQL("select * from "+prefix+"_emu_events where event_sha1='"
				+ event_sha1 + "'");
		// System.out.println("select * from libx_pages where id='" + id + "'");
		// ResultSet rs = executeQuerySQL("select * from libx_pages");
		try {
	 ...
	 * @return
	 */
	public boolean insertEvent(String event_sha1, Long time, String ev,
			String url, String ref, String ip, Integer userID,
			String content_sha1, Integer cx, Integer cy, Integer x, Integer y,
			Integer scrlX, Integer scrlY, Integer iw, Integer ih, Integer ow,
			Integer oh, Integer scrlW, Integer scrlH, String dom_path,
			String targ_id, Boolean is_doc_area, Integer duration,
			String select_text, String button) {
	...
	String _sql_insert_mouse = "insert into emu_mouses (" + "event_id, "
			+ "time, " + "ev, " + "page_id, " + "ip, " + "user_id, " + "x, "
			+ "y, " + "xdist, " + "ydist, " + "is_serp, " + "res_rand, "
			+ "task, " + "founded, " + "has_image, " + "has_fixation "
			+ ") values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean insertMouse(Integer eventID, Long time, String ev,
			Integer pageID, String ip, Integer userID, Integer x, Integer y,
			Integer xdist, Integer ydist, Boolean isSERP, Boolean res_rand,
			String task, Boolean founded, Boolean has_image,
	...
	public static void stmtSetInt(PreparedStatement stmt, int index, Integer val)
			throws SQLException {
		if (val != null) {
	...
	public static void stmtSetBoolean(PreparedStatement stmt, int index,
	...
	public static void stmtSetString(PreparedStatement stmt, int index,
			String val) throws SQLException {
		stmt.setString(index, val);
	}

	String _sql_insert_content = "insert into emu_contents ("
			+ "content_sha1, "
			// + "time, "
			// + "url, "
			+ "data, "
	...
	public boolean insertContent(String content_sha1, Long time, String url,
			String data, String type, int length) {
